...
9. Evidence에 '사용자 표시 금액'이 있으면 그 금액 문자열을 최종 답변에 그대로 사용하고, 원문 값과 단위를 조합해 새로운 금액 표현을 만들지 마세요.
10. '사용자 표시 금액', '정규화된 최종 금액' 같은 Evidence 항목명이나 내부 필드명은 최종 답변 문장에 노출하지 마세요.
"""
...
def build_grounded_answer_prompt(query: str, pack: EvidencePack) -> str:
    ...
    return "\n".join(
        (
            ...
            "- '사용자 표시 금액'이 있으면 해당 금액 문자열만 그대로 사용하세요.",
            "- '사용자 표시 금액'이 없고 정규화된 최종 금액만 있으면 그 값을 사용하세요.",
