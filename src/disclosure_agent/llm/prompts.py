...
36. 사용자가 묻지 않은 인접 주제의 시장 전망, 산업 설명, 일반 사업 전략으로 답변 분량을 채우지 마세요. 직접 질문에 필요한 근거만 우선 사용하세요.
37. 비교 질의는 각 대상 또는 연도의 핵심 차이를 먼저 압축해 제시하고, 원문 배경을 길게 재서술하지 마세요. 질문 범위 밖의 미래 전망은 비교 결론에 꼭 필요한 경우에만 최소한으로 포함하세요.
38. 동일한 사실이나 전략을 서로 다른 제목·사업부·목록 아래 반복해서 쓰지 마세요. 하나의 사실은 가장 직접적으로 뒷받침되는 한 위치에서만 설명하세요.
39. 번호 목록을 사용하면 1부터 순차적으로 작성하고, 이미 설명한 항목을 '기타' 등 다른 제목으로 다시 분류하지 마세요.
40. Evidence 원문을 길게 복사하거나 표·제품 사양·연혁을 그대로 나열하지 마세요. 질문에 필요한 사실을 골라 자연스러운 문장으로 요약·통합하세요.
41. 요약·비교·분류는 Evidence가 직접 뒷받침하는 범위에서 적극적으로 수행하세요. 단순 발췌에 그치지 말고 '무엇이 핵심인지', '어떤 차이가 있는지'를 설명하되 Evidence에 없는 원인·목적·전망을 새로 만들지는 마세요.
42. 답변은 원문 재현이 아니라 분석 결과여야 합니다. 핵심 사실 → 의미 있는 정리/비교 → 필요한 정보 한계 순서로 구성하고, 불필요한 세부 사양과 배경 문장은 생략하세요.
...
def _query_specific_requirements(query: str, pack: EvidencePack) -> tuple[str, ...]:
    ...
    if asks_investment_direction_or_purpose:
        requirements.append(
            ...
            "그 범위가 제한적이라고 밝히고, 일반 사업전략·시장 전망으로 답변을 보충하지 마세요."
        )
        asks_related_context = any(
            marker in compact for marker in ("전략", "전망", "시장", "사업환경", "수요")
        )
        if not asks_related_context:
            requirements.append(
    ...
    if "시스템반도체" in compact:
        ...
        if "투자" in compact and "목적" in compact:
            requirements.append(
                "- '투자 목적'은 Evidence가 '위한 투자', '투자 목적'처럼 투자와 목적의 관계를 "
                "직접 표현하는 경우에만 사용하세요. 시장 수요, 성장률, 가격 경쟁, 수익 구조, "
                "수주 전략 같은 일반 사업 맥락은 명시적 투자 연결이 없으면 답변에서 제외하세요."
            )
            requirements.append(
                "- 사용자가 투자 방향·목적을 물었고 금액이나 규모를 요청하지 않았다면 이미 "
                "집행된 전체 시설투자 금액을 별도 실적으로 덧붙이지 마세요. 직접 확인되는 "
                "투자 방향·목적이 하나뿐이면 그 한계를 그대로 밝히고 항목 수를 늘리지 마세요."
            )
            requirements.append(
...
def build_grounded_answer_prompt(query: str, pack: EvidencePack) -> str:
    ...
    requirements = (
        ...
        "- 공시 원문을 그대로 길게 복사하지 말고, 질문에 필요한 사실을 선별해 자연스러운 분석 문장으로 요약·통합하세요.",
        "- 단순 사실 나열보다 Evidence가 직접 허용하는 범위에서 핵심 의미와 비교 포인트를 정리하세요. 다만 근거에 없는 원인·목적·전망은 만들지 마세요.",
