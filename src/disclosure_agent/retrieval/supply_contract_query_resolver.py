"""Resolve one company and one formation year from supply-contract lifecycle questions."""
...
class SupplyContractQueryTarget:
    """Resolved company/year target for one contract-lifecycle question."""
...
def resolve_supply_contract_query_target(
    session: Session,
    *,
    query: str,
    fallback_company: str | None = None,
    fallback_year: int | None = None,
) -> SupplyContractQueryTarget:
    """Resolve exactly one company and one original contract year without guessing."""
    ...
    if contains_placeholders and not companies:
        return SupplyContractQueryTarget(
            status="UNRESOLVED",
            company_name=None,
